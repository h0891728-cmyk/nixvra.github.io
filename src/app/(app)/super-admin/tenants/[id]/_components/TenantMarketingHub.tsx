...
/* ═══════════════════════════════════════════════════════════════
   MAIN HUB
   ═══════════════════════════════════════════════════════════════ */
export default function TenantMarketingHub({ tenantId }: { tenantId: string }) {
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div>
          ...
          <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', margin: '0.25rem 0 0' }}>
            Manage social posts, ad campaigns, and WhatsApp messaging for this tenant.
          </p>
        </div>
        {/* Integration quick status pills */}
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', alignSelf: 'center' }}>Integrations:</span>
          <ConnBadge connected={metaConnected} />
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', alignSelf: 'center' }}>Meta</span>
          <ConnBadge connected={waConnected} />
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', alignSelf: 'center' }}>WhatsApp</span>
        </div>
      </div>
          ...
          {loading ? <Spinner label="Loading messages..." /> : messages.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: 420, overflowY: 'auto' }}>
              {messages.map((msg: any) => {
                ...
                const succeeded = msg.status === 'PROCESSED'
